...
namespace EditorAnimationPreview
{
	public static class AnimationPathSceneUI
	{
		...
		public static void OpenSceneTool(GameObject go)
		{
			...
			if (keepShow)
			{
				SceneView.onSceneGUIDelegate = (SceneView.OnSceneFunc) Delegate.Combine(SceneView.onSceneGUIDelegate,
					new SceneView.OnSceneFunc(OnSceneViewGUI));
			}
		...
		private static void CloseSceneTool()
		{
			...
			AnimationUtility.onCurveWasModified -= OnCurveWasModified;
			SceneView.onSceneGUIDelegate = (SceneView.OnSceneFunc) Delegate.RemoveAll(SceneView.onSceneGUIDelegate,
				new SceneView.OnSceneFunc(OnSceneViewGUI));
			SceneView.RepaintAll();
		}
		...
		private static bool SetPointTangent(int pointIndex, Vector3 worldTangent, bool isInTangent)
		{
			...
			string inPath =
				AnimationUtility.CalculateTransformPath(activeGameObject.transform, activeRootGameObject.transform);
			Type inType = typeof(Transform);
			...
			AnimationPathPoint.ModifyPointTangent(pathPoint, nextPathPoint, offset, isInTangent, curveX, curveY,
				curveZ);
